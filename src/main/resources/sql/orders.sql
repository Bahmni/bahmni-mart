...
FROM (SELECT
      ...
      FROM orders
        ...
        INNER JOIN encounter_type et ON e.encounter_type = et.encounter_type_id AND et.retired = FALSE
      WHERE order_type_id = :orderTypeId) AS o
...
FROM (SELECT
      ...
      FROM orders
        INNER JOIN encounter e ON orders.encounter_id = e.encounter_id AND e.voided = FALSE
        INNER JOIN encounter_type et ON e.encounter_type = et.encounter_type_id AND et.retired = FALSE
      WHERE order_type_id = :orderTypeId) AS o1
