select CONCAT('\"',cv.concept_full_name,'\"') as fully_specified_name,CONCAT('\"',cv.concept_short_name,'\"') as question,CONCAT('\"',cv.description,'\"') as description, crtv.code as question_code, CONCAT('\"',cn.name,'\"') as answer,crtv_answer.code as answer_code
