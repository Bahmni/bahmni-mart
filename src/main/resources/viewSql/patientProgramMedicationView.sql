SELECT
  ...
  EXTRACT(YEAR FROM (SELECT age( md.start_date, TO_TIMESTAMP(pd.birthyear, 'yyyy')))) AS age_at_medication,
  age_group(md.start_date, TO_TIMESTAMP(pd.birthyear, 'yyyy')) AS age_group_at_medication,
  pd.dead,
  pa.*,
