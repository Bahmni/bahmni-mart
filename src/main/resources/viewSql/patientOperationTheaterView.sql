SELECT
  ...
  pd.birthyear               AS birth_year,
   EXTRACT(YEAR FROM (SELECT age( sb.block_starttime, TO_TIMESTAMP(pd.birthyear, 'yyyy')))) AS age_at_surgery,
  age_group(sb.block_starttime, TO_TIMESTAMP(pd.birthyear, 'yyyy')) AS age_group_at_surgery,
  pd.dead,
  pa.*,
