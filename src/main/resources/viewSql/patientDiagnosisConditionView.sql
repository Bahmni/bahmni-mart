SELECT
  ...
  EXTRACT(YEAR FROM (SELECT age( c.onset_date, TO_TIMESTAMP(pd.birthyear, 'yyyy')))) AS age_at_condition,
  age_group(c.onset_date, TO_TIMESTAMP(pd.birthyear, 'yyyy')) AS age_group_at_condition,
  pd.dead,
  pa.*,
  c.condition_id,
