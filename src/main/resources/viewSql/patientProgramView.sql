SELECT
  ...
  EXTRACT(YEAR FROM (SELECT age( ppd.date_enrolled, TO_TIMESTAMP(pd.birthyear, 'yyyy')))) AS age_at_program,
  age_group(ppd.date_enrolled, TO_TIMESTAMP(pd.birthyear, 'yyyy')) AS age_group_at_program,
  pa.state_province,
  pd.gender,
  pd.birthyear,
  pd.birthtime,
