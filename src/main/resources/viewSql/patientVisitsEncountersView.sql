SELECT
  ...
  pd.gender,
  pd.birthyear,
  EXTRACT(YEAR FROM (SELECT age( pvd.visit_start_date, TO_TIMESTAMP(pd.birthyear, 'yyyy')))) AS age_at_visit,
  age_group(pvd.visit_start_date, TO_TIMESTAMP(pd.birthyear, 'yyyy')) AS age_group_at_visit,
  pd.birthtime,
