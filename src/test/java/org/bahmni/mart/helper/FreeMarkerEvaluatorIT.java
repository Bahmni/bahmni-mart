...
public class FreeMarkerEvaluatorIT extends AbstractBaseBatchIT {
    ...
    public void ensureParentWithDepth1IsConstructed() {
        ...
        assertEquals("SELECT obs0.obs_id , obs1.obs_id as parent_obs_id FROM obs obs0 INNER JOIN obs obs1 on" +
                        " (obs1.obs_id = obs0.obs_group_id and obs1.voided = 0) WHERE obs0.concept_id =10 " +
    ...
    public void ensureParentWithDepth2IsConstructed() {
        ...
        assertEquals("SELECT obs0.obs_id , obs1.obs_id as parent_obs_id FROM obs obs0 INNER JOIN obs obs1 " +
                "on (obs1.obs_id = obs0.obs_group_id and obs1.voided = 0) INNER JOIN obs obs2 " +
                "on (obs2.obs_id = obs1.obs_group_id and obs2.voided = 0) " +
                "WHERE obs0.concept_id =10 AND obs0.voided = 0 AND obs1.concept_id =1", sql.trim());
    }
    ...
    public void ensureParentWithDepthIsMoreThan2Constructed() {
        ...
        assertEquals("SELECT obs0.obs_id , obs1.obs_id as parent_obs_id FROM obs obs0 INNER JOIN obs obs1 " +
                "on (obs1.obs_id = obs0.obs_group_id and obs1.voided = 0) INNER JOIN obs obs2 " +
                "on (obs2.obs_id = obs1.obs_group_id and obs2.voided = 0) INNER JOIN obs obs3 " +
                "on (obs3.obs_id = obs2.obs_group_id and obs3.voided = 0) " +
                "WHERE obs0.concept_id =12 AND obs0.voided = 0 AND obs1.concept_id =10", sql.trim());
    }
}
