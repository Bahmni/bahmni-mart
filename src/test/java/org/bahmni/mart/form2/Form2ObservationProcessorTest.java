...
public class Form2ObservationProcessorTest {
    ...
    @Test
    public void shouldProcessGivenDataToCodedObs() {
        BahmniForm form = mock(BahmniForm.class);
        when(form.getDepthToParent()).thenReturn(0);
        form2ObservationProcessor.setForm(form);
        Map<String, Object> obsRow = getCodedObsRow();
        Map<String, String> fieldNameAndFullySpecifiedNameMap = new HashMap<>();
        fieldNameAndFullySpecifiedNameMap.put("Coded concept", "Coded concept in Given locale");
        fieldNameAndFullySpecifiedNameMap.put("Concept Answer", "Concept Answer in Given Locale");
        when(form.getFieldNameAndFullySpecifiedNameMap()).thenReturn(fieldNameAndFullySpecifiedNameMap);
        ...
        Concept field = obs.getField();
        assertEquals(Integer.valueOf(7), field.getId());
        assertEquals("Coded concept in Given locale", field.getName());
        assertEquals("Concept Answer in Given Locale", obs.getValue());
        assertEquals("2015-02-01 00:00:00", obs.getObsDateTime());
        assertEquals("2016-11-09 7:31:43", obs.getDateCreated());
        assertEquals("8", obs.getLocationId());
        assertEquals("Registration", obs.getLocationName());
    ...
    private Map<String, Object> getCodedObsRow() {
        Map<String, Object> obsRow = new HashMap<>();
        obsRow.put("encounter_id", "abc123");
        obsRow.put("patientId", "def789");
        obsRow.put("conceptId", 7);
        obsRow.put("id", 97);
        obsRow.put("value", "Concept Answer");
        obsRow.put("conceptName", "Coded concept");
        obsRow.put("obsDateTime", "2015-02-01 00:00:00");
        obsRow.put("dateCreated", "2016-11-09 7:31:43");
        obsRow.put("locationId", "8");
        obsRow.put("locationName", "Registration");
